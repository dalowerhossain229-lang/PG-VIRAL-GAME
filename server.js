...
const MAIN_SITE_URL = "htt//betlover247.onrender.com"; 
...
app.post('/api/slot-spin', async (req, res) => {
    ...
    while (isCascadeContinuing && currentCascadeIndex < cascadeMultiplierLadder.length) {
        ...
        // 🚀 [🔒 নিখুঁত অল-সিম্বল সিকোয়েন্সিয়াল উইন এনালাইজার 🔒]
        let maxStepMultiplier = 0;
        let stepBestMatchCount = 0;
        let stepWinningSymbol = null;
        let stepWinningRows = [];

        // ১ম কলামের অনন্য ছবিগুলো ফিল্টার করে চেক করা হবে
        ...
        for (let baseSym of col0UniqueSymbols) {
            ...
            // ১ম কলামের প্রথম প্রাপ্ত ইনডেক্স ট্র্যাক
            rowPositions.push(sym[0].indexOf(baseSym) !== -1 ? sym[0].indexOf(baseSym) : sym[0].indexOf("BANDIT_WILD"));

            // 🔍 ২য় থেকে ৬ষ্ঠ কলাম নিখুঁত লিনিয়ার চেইন চেক (মাঝখানে গ্যাপ থাকলে কন্ডিশন ইনস্ট্যান্ট ব্রেক করবে)
            for (let colIdx = 1; colIdx < 6; colIdx++) {
                if (sym[colIdx].includes(baseSym) || sym[colIdx].includes("BANDIT_WILD")) {
                    matchCount++;
                    let targetIdx = sym[colIdx].indexOf(baseSym) !== -1 ? sym[colIdx].indexOf(baseSym) : sym[colIdx].indexOf("BANDIT_WILD");
                    rowPositions.push(targetIdx);
                } else {
                    break; // 🔒 গ্যাপ থাকলে চেইন এখানেই লক এবং টার্মিনেট হবে!
                }
            }

            // ৩ বা তার বেশি ম্যাচ হলে ওডস এবং মাল্টিপ্লায়ার হিসাব
            if (matchCount >= 3) {
                ...
                // 🎯 পাশাপাশি একই সোজা লাইনে জোড়া লাগলে ৪ গুণ বিগউইন বুস্টার
        ...
        if (maxStepMultiplier > 0) {
            ...
            if ((reqAmount === 1 || reqAmount === 2) && Math.random() <= 0.05) {
                maxStepMultiplier = parseFloat((Math.random() * (1200 - 300) + 300).toFixed(2));
                    ...
                    // 🛡️ হাই বেট ব্যালেন্স সেফগার্ড প্রোটেকশন
        if (reqAmount > 100) {
        ...
        isCascadeContinuing = false; // কোনো উইন না থাকলে ক্যাস্কেড সম্পূর্ণ বন্ধ
    }
}
